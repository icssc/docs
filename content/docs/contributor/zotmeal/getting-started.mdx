...
<Step>
Change your node version to the one specified in the .nvmrc by running `nvm use` or `fnm use`.
</Step>
   
<Step>
Navigate to the root directory and install the dependencies by running `cd ZotMeal && pnpm install`.
</Step>
...
<Step>
Start local development by running `pnpm dev` in the root directory.
...
<Step>
View the local website at [http://localhost:3001](http://localhost:3000) 
